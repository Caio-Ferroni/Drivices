<x-header_admin pageTitle="Ofertas" breadcrumb="Editar">
<x-content>

    <div class="d-flex align-items-center gap-3 mb-4">
        <a href="{{ route('pedidos.show', $oferta->pedido_id) }}" class="btn-dash-back">
    ...
    <div class="dash-card">
        ...
        <form action="{{ route('ofertas.update', $oferta) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="dash-card-body">
                ...
                <div class="dash-show-user-row">
                    <div class="dash-td-ava dash-td-ava-lg">
                        {{ strtoupper(substr($oferta->pedido->user->name, 0, 2)) }}
                    </div>
                    <div>
                        <div class="dash-show-user-name">{{ $oferta->pedido->user->name }}</div>
                        <div class="dash-td-sub">Pedido #{{ $oferta->pedido_id }} · {{ Str::limit($oferta->pedido->descricao, 60) }}</div>
                    </div>
                    @if($oferta->pedido->emergencia)
